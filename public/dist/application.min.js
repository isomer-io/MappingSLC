...
"use strict";angular.module("contacts").config(["$stateProvider",function($stateProvider){$stateProvider.state("createContact",{url:"/contact-us",templateUrl:"modules/contacts/client/views/contact-us.client.view.html"}).state("editContact",{url:"/contacts/:contactId/edit",templateUrl:"modules/contacts/client/views/edit-contact.client.view.html"}).state("aboutMappingSlc",{url:"/about-the-project",templateUrl:"modules/contacts/client/views/about-the-project.client.view.html"}).state("aboutUs",{url:"/about-us",templateUrl:"modules/contacts/client/views/about-us.client.view.html"})}]);
...
"use strict";angular.module("users").controller("ContributorController",["$scope","$animate","$location","Authentication","GetContributors","$stateParams","$http","$modal","$window","Lightbox",function($scope,$animate,$location,Authentication,GetContributors,$stateParams,$http,$modal,$window,Lightbox){$scope.contributors=null,$scope.init=function(){GetContributors.contributors()},GetContributors.contributors().success(function(contributorsData){console.log("contributorsData: ",contributorsData),$scope.contributors=contributorsData,console.log("$scope.contributors: ",$scope.contributors)}).error(function(errorData){console.log("errorData: ",errorData)}),$scope.changeView=function(view){$location.path(view)},$scope.Lightbox=Lightbox,$scope.images=[],$scope.getImages=function(){var i;for(i=0;i>$scope.contributors.length;i++)$scope.images.push($scope.contributors[i].profileImageURL)}}]);
