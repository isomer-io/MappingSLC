...
"use strict";angular.module("users").factory("Users",["$resource",function($resource){return $resource("/api/v1/users",{},{update:{method:"PUT"}})}]),angular.module("users").factory("Admin",["$resource",function($resource){return $resource("/api/v1/users/:userId",{userId:"@_id"},{update:{method:"PUT"}})}]);
...
"use strict";angular.module("users").controller("EditProfileController",["$scope","$http","$location","Users","UserData","$stateParams","Authentication","UtilsService",function($scope,$http,$location,Users,UserData,$stateParams,Authentication,UtilsService){$scope.currentUser=Authentication.user,console.log("Authentication.users: ",Authentication.user),UtilsService.cssLayout(),$scope.updateUserProfile=function(isValid){if($scope.success=$scope.error=null,!isValid)return $scope.$broadcast("show-errors-check-validity","userForm"),!1;console.log("here");var user=new Users($scope.user);user.$update(function(response){$scope.$broadcast("show-errors-reset","userForm"),$scope.success=!0,Authentication.user=response,console.log("here 2 w response",response)},function(response){console.log("here 3 w response",response),$scope.error=response.data.message,console.log("error!\n",$scope.error)})},$scope.toggleEdit=!1,$scope.toggleId=0,$scope.toggleEditFn=function(editNum){$scope.toggleEdit=!$scope.toggle,$scope.toggleId=editNum},$scope.find=function(){$scope.users=Users.query()},$scope.find=function(){$scope.users=Users.query($scope.query)},$scope.findOne=function(){$scope.user=UserData.get({userId:$stateParams.userId}),console.log("$scope.users: ",$scope.user)}}]);
